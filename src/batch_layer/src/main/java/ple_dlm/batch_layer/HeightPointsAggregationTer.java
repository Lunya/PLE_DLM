...
public class HeightPointsAggregationTer {
	...
	public static void aggregate(SparkConf conf, Configuration hbaseConf, int maxZoomLevel) {
		...
		JavaPairRDD<Tuple2<Double, Double>, byte[]> images = points.groupByKey().mapToPair((region) -> {
			...
			final double longitude = region._1()._2();
			byte res[] = new byte[2 * imageSize * imageSize];
			for (short point : region._2()) {
		...
		Function<Iterable<Short>, Short> mapMap = (pts) -> {
			...
			return (short)((double)mean / (double)count);
		};
		JavaPairRDD<Tuple2<Double, Double>, Short> keyGroupedPoints = points.groupByKey().mapValues(mapMap);
		keyGroupedPoints.cache();
		// insert points in database
		
		// iterate on values to generate smallest levels
		JavaPairRDD<Tuple2<Double, Double>, Short> lastLevel = keyGroupedPoints;
		for (int zoomLevel = maxZoomLevel-1; zoomLevel >= 0; --zoomLevel) {
			final double angleStep1 = 180.0 / Math.pow(2, zoomLevel);
			
			JavaPairRDD<Tuple2<Double, Double>, Short> currentLevel = lastLevel.mapToPair((point) -> {
				final double latitude = Math.floor(point._1()._1()/angleStep1) * angleStep1;
				final double longitude = Math.floor(point._1()._2()/angleStep1) * angleStep1;
				return new Tuple2<Tuple2<Double, Double>, Short>(
						new Tuple2<Double, Double>(latitude, longitude),
						point._2());
			});
			currentLevel = currentLevel.groupByKey().mapValues(mapMap);
			lastLevel = currentLevel.cache();
			
			// insert points in database
			// INSERT lastLevel
		}
		JavaPairRDD<String, Tuple3<Double, Double, Short>> geohashPoints = points.mapToPair((point) -> {
			return new Tuple2<String, Tuple3<Double, Double, Short>>(
					GeoHash.encodeHash(
							point._1()._1(),
							point._1()._2(),
							8),
					new Tuple3<Double, Double, Short>(
							point._1()._1(),
							point._1()._2(),
							point._2()));
		});
	...
	private static void saveRDDToDatabase(JavaPairRDD<Tuple2<Double, Double>, byte[]> rdd, byte zoomLevel) {
		JavaPairRDD<String, byte[]> rddGeohashed = rdd.mapToPair((image) -> {
			return new Tuple2<String, byte[]>(GeoHash.encodeHash(
					image._1()._1(),
					image._1()._2(),
					8), image._2());
		});
		JavaPairRDD<ImmutableBytesWritable, KeyValue> hbasePuts = rddGeohashed.sortByKey().flatMapToPair((image) -> {
			return Collections.singletonList(
					new Tuple2<ImmutableBytesWritable, KeyValue>(
							new ImmutableBytesWritable(Bytes.toBytes(image._1() + HBaseSetup.HEIGHT_FAMILY)),
							new KeyValue(
									image._1().getBytes(),
									HBaseSetup.HEIGHT_FAMILY.getBytes(),
									Byte.toString(zoomLevel).getBytes(),
									image._2()))).iterator();
		});
		hbasePuts.saveAsNewAPIHadoopFile("/tmp/DLM", ImmutableBytesWritable.class, KeyValue.class, HFileOutputFormat2.class, 
				job.getConfiguration());
