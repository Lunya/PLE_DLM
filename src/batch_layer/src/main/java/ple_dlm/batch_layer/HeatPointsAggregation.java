...
public class HeatPointsAggregation {
	...
	public static void aggregate(SparkConf conf, Configuration hbaseConf) {
		...
		for (int zoom=0; zoom<5; zoom++) {
			...
			String tableName = HBaseSetup.TABLE_BASENAME + "_" + zoom;
			...
			JavaPairRDD<ImmutableBytesWritable, KeyValue> hbasePuts = rddImagesRegionMatrixed.sortByKey().flatMapToPair((x) -> {
	            ...
	            result.add(new Tuple2<ImmutableBytesWritable, KeyValue>(
	                    ...
	                    new KeyValue(
	                    		...
	                    		HBaseSetup.HEIGHT_COL.getBytes(), // column
	                    		x._2()))); // value
	            
	            return result.iterator();
	        });
			hbasePuts.saveAsNewAPIHadoopFile("/tmp/DLM_" + zoom, ImmutableBytesWritable.class, KeyValue.class, HFileOutputFormat2.class, job.getConfiguration());
